		    ...
		    uint32_t init_check)
{
	...
	if (init_check) {
		reg_rdata = mmio_read_32((uintptr_t)&mt8183_pwrap->wacs2_rdata);
	...
	return_value = wait_for_state_idle(TIMEOUT_WAIT_IDLE,
				&mt8183_pwrap->wacs2_rdata,
				&mt8183_pwrap->wacs2_vldclr,
				0);
	if (return_value != 0) {
	...
	mmio_write_32((uintptr_t)&mt8183_pwrap->wacs2_cmd, wacs_cmd);
	if (write == 0) {
		if (rdata == NULL) {
		...
		*rdata = ((reg_rdata >> RDATA_WACS_RDATA_SHIFT)
		...
		mmio_write_32((uintptr_t)&mt8183_pwrap->wacs2_vldclr, 1);
	}
FAIL:
	return return_value;
}
